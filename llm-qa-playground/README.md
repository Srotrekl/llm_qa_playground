...
*Allure overview against `llama3.2:1b`. The 26 "failed" cases are **detected LLM vulnerabilities** — prompts where the model leaked PII, complied with a jailbreak, or hallucinated a fact. Surfacing these is the whole point of the framework; each is logged as a documented finding, not a broken test.*
